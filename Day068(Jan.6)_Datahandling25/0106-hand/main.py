...
def main():
    ...
    net.fc = nn.Linear(512, 3) # 모델을 꼭 print()를 사용해서 확인을 주어야 한다.
    # print(net)
    ...
    optimizer = torch.optim.AdamW(net.parameters(), lr=0.0001) # net의 파라메타를 넣어줘야 한다.
    epochs = 20
    ...
    if os.path.exists(save_path):
        best_val_acc = max(pd.read_csv("./resnet34.csv")["Accuracy"].tolist())
    ...
    for epoch in range(epochs):
        ...
        if epoch == epochs - 1:
            dfForAccuracy.to_csv("./resnet34.csv", index=False)
